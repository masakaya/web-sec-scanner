...
Examples:
  ...
  %(prog)s automation http://example.com \\
    ...
    scan_group.add_argument(
        ...
        action="append",
        dest="addons",
        help="ZAP AddOn to install (can be used multiple times, e.g., --addon jwt --addon graphql). Default: authhelper, ascanrules",
    )
...
def validate_scan_config(args: argparse.Namespace) -> ScanConfig:
    ...
    config = ScanConfig(
        ...
        addons=args.addons or ["authhelper", "ascanrules"],
